...
{
    printk(KERN_ERR
        "[bytedev:] Bytedev write response!");
    return size;
}
...
static int bytedev_get_unuse_minor_num(void)
{
    int ret = -1;
                            ...
                            const struct pci_device_id *id)
{
    ...
    bytedev->mmio_addr = pci_iomap(pdev, 0, BYTEDEV_MMIO_SIZE);
    if (!bytedev->mmio_addr) {
        printk(KERN_ERR "Cannot iomap for MMIO space, abort.");
        err = -ENOMEM;
        goto err_out_free_region;
    }

    /* iomap for pmio space */
    bytedev->pmio_addr = pci_iomap(pdev, 1, BYTEDEV_PMIO_SIZE);
    if (!bytedev->pmio_addr) {
        printk(KERN_ERR "Cannot iomap for PMIO space, abort.");
        err = -ENOMEM;
        goto err_out_iounmap_mmio;
    }

    /* register device node */
    minor_num = bytedev_get_unuse_minor_num();
    if (minor_num < 0) {
        printk(KERN_ERR "[bytedev:] bytedev amount limits!");
        goto err_dev_register_fail;
    }
    else if (minor_num == 0) {
...
err_dev_register_fail:
    pci_iounmap(pdev, bytedev->pmio_addr);
